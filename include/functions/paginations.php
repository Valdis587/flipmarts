 ...
 */
 ...
 function wp_main_pagination( $args = array() ) {
    ...
    $defaults = array(
        ...
        'before_output'   => '<nav class="pagination"><ul class="pagination__list">',
        'after_output'    => '</ul></nav>'
    );
    ...
    if ( !empty($min) && !empty($max) ) {
        for( $i = $min; $i <= $max; $i++ ) {
            if ($page == $i) {
                $echo .= '<li  class="active"><a class="pagination__active">' . str_pad( (int)$i, 2, '0', STR_PAD_LEFT ) . '</a></li>';
            } else {
