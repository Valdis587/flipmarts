...
class LastNews_Widget extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            ...
            esc_html__( 'Последние новости Flipmarts', 'smarkets' ), // Name
...
<ul class="sidebar__wiget-last-news-list">
                    <?php
                    while ( $r->have_posts() ) {
                        global $product;
                        $r->the_post(); ?>
                     <li class="sidebar__wiget-last-news-item">
                         <?php
                         $id = get_post_thumbnail_id();
                         $main=wp_get_attachment_image_src( $id, 'last-blog' );
                         ?>
                         <a href="<?php the_permalink(); ?>"><img src="<?php echo $main[0]; ?>" alt="<?php the_title(''); ?>"></a>
                     <a href="<?php the_permalink(); ?>"><h4 class="sidebar__wiget-last-news-title"><?php the_title(''); ?></h4></a>
                    </li>
                   <?php } ?>
                    </ul>
 </div>
